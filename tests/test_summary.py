...
def _table_lines(md: str) -> list[str]:
    """Só as linhas de DEAL (17 colunas = 18 pipes); exclui cabeçalho/separador e a
    tabela de contagens do modo diagnóstico (4 colunas)."""
    return [ln for ln in md.splitlines()
            if ln.startswith("|") and ln.count("|") == 18
...
def test_header_counts_scope_funnel_and_thresholds():
    ...
    assert "condições: NM, LP, EX-NM; EX-NM sem preço presumido; LP só com referência LP" in md
    assert "idiomas: en" in md
