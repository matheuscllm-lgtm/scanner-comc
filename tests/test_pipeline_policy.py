...
def test_slab_sales_median_reference_can_be_ok(index, monkeypatch):
    monkeypatch.setattr(pl, "graded_reference",
                        lambda *a, **k: GradedRef(price=95.0, grade_key="vendas BGS 9.5 (n=4)",
                                                  url="https://pc/x", method="sales", n_sales=4))
    sc = pl.Scanner(_settings())
    ...
    assert d.as_row()["price_field"] == "vendas BGS 9.5 (n=4)"


def test_slab_generic_bucket_is_triage_only(index, monkeypatch):
    monkeypatch.setattr(pl, "graded_reference",
                        lambda *a, **k: GradedRef(price=25.0, grade_key="GRADE 9.5",
                                                  url="https://pc/x", method="proxy"))
    sc = pl.Scanner(_settings())
    d = sc.process_listing(_slab(15.0, "TAG 9.5", "TAG"), index, CTX, pl.KIND_SLAB)
    assert d is not None and d.status == "MATCH_REVIEW"
    assert "ref~proxy:GRADE 9.5" in d.review_reasons and d.ref_source == "pricecharting-proxy"
