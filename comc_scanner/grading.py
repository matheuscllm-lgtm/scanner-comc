...
# "Grade 9" e "Grade 9.5" são buckets GENÉRICOS (misturam certificadoras) — nunca
# coluna exata: a referência real vem da mediana de vendas da mesma certificadora+nota.
_PC_DIRECT = {"PSA 10", "BGS 10", "CGC 10 PRISTINE", "SGC 10", "TAG 10", "ACE 10"}
...
def pc_price_key(grade: Grade) -> tuple[str | None, bool]:
    ...
    if grade.value in (9.0, 9.5):
        # PC agrega todas as certificadoras em "Grade 9" / "Grade 9.5": serve só para
        # TRIAGEM (MATCH_REVIEW); a oportunidade real vem da mediana de vendas da
        # mesma certificadora+nota (pricecharting_client.graded_reference).
        return f"GRADE {grade.value:g}", True
    return None, True
